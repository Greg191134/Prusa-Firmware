  ...
  if (code_seen("M117")) { //moved to highest priority place to be able to to print strings which includes "G", "PRUSA" and "^"
	...
	else if (strncmp_P(CMDBUFFER_CURRENT_STRING, PSTR("TMC_"), 4) == 0)
	{
		if (strncmp_P(CMDBUFFER_CURRENT_STRING + 4, PSTR("SET_WAVE_"), 9) == 0) //! TMC_SET_WAVE_
		{
			...
			if (axis < 4)
			{
				uint16_t fac = (uint16_t)strtol(CMDBUFFER_CURRENT_STRING + 14, NULL, 10);
        ...
        else if (strncmp_P(CMDBUFFER_CURRENT_STRING + 4, PSTR("SET_DMCC_"), 9) == 0) //! TMC_SET_DMCC_
		{
			...
			if (axis < 4)
			{
				...
				{
					coil_a_current = (int16_t)strtol(CMDBUFFER_CURRENT_STRING + 14, &str_end, 10);
					coil_b_current = 248 - coil_a_current;
					if (str_end && *str_end)
						coil_b_current = (int16_t)strtol(str_end, &str_end, 10);
				}
		...
		else if (strncmp_P(CMDBUFFER_CURRENT_STRING + 4, PSTR("SET_LINR_"), 9) == 0) //! TMC_SET_LINR_
		{
			...
			if (axis < 4)
			{
				...
				if (CMDBUFFER_CURRENT_STRING[14])
				{
					step = (uint8_t)strtol(CMDBUFFER_CURRENT_STRING + 14, &str_end, 10);
					if (str_end && *str_end)
						division = (uint8_t)strtol(str_end, &str_end, 10);
				}

				printf_P(PSTR("Step=%d division=%d\n"), step, division);
				coil_a_current = (int16_t)((248) * sin((2*PI*step)/(division * 4)) + 0.5) - 1;
				coil_b_current = (int16_t)((248) * cos((2*PI*step)/(division * 4)) + 0.5) - 1;
				tmc2130_set_direct_mode_current(axis, coil_a_current, coil_b_current);
			}
		}
		...
		else if (strncmp_P(CMDBUFFER_CURRENT_STRING + 4, PSTR("SET_PHASE_"), 10) == 0) //! TMC_SET_PHASE_
		{
			...
			if (axis < 4)
			{
				uint16_t phase = (uint16_t)strtol(CMDBUFFER_CURRENT_STRING + 15, NULL, 10);
		...
		else if (strncmp_P(CMDBUFFER_CURRENT_STRING + 4, PSTR("SET_STEP_"), 9) == 0) //! TMC_SET_STEP_
		{
			...
			if (axis < 4)
			{
				...
				uint16_t mscnt1 = tmc2130_rd_MSCNT(axis);
				uint32_t mscuract = tmc2130_rd_MSCURACT(axis);
				int16_t coil_a_current = (mscuract & 0x100) ? - (256 - (mscuract & 0xFF)) : mscuract & 0xFF ;
				int16_t coil_b_current = ((mscuract >> 16) & 0x100) ?  - (256 - ((mscuract >> 16) & 0xFF)) : (mscuract >> 16) & 0xFF ;
        ...
        else if (strncmp_P(CMDBUFFER_CURRENT_STRING + 4, PSTR("DO_STEPS_"), 9) == 0) //! TMC_DO_STEPS_
		{
			...
			if (axis < 4)
			{
				int8_t steps = (int8_t)strtol(CMDBUFFER_CURRENT_STRING + 14, NULL, 10);
				uint8_t dir = tmc2130_get_inv(axis)?0:1;
				if (steps < 0)
				{
					dir ^= 1;
					steps = -steps;
		...
		else if (strncmp_P(CMDBUFFER_CURRENT_STRING + 4, PSTR("TEST_ADL_"), 9) == 0) //! TMC_TEST_ADL_
		{
			...
			if (axis < 4)
			{
				...
				uint8_t  shifted_entry = 0;
				uint8_t  action = 0;
				int8_t   dir = 1;
				char* str_end = 0;
				if (CMDBUFFER_CURRENT_STRING[14])
				{
					mscnt0 = (uint16_t)strtol(CMDBUFFER_CURRENT_STRING + 14, &str_end, 10) & 0x3FF;
					if (str_end && *str_end)
						dir = (int8_t)strtol(str_end, &str_end, 10);
				}

				tab_entry = e_lin_curve[ mscnt0 >> 4 ];
				tab_index = mscnt0 >> 4;
				shifted_entry = (e_lin_curve[ mscnt0 >> 4 ] >> ((mscnt0 & 0xF) << 1));
				action = (e_lin_curve[ mscnt0 >> 4 ] >> ((mscnt0 & 0xF) << 1)) & 0x3;
		  	   	mscnt1 = mscnt0 + dir;
		  	    mscnt1 &= 0x3FF;

				printf_P(PSTR("tab_entry=%lx tab_index=%d shifted_entry=%x action=%d mscnt0=%d mscnt1=%d dir=%d\n"), tab_entry, tab_index, shifted_entry, action, mscnt0, mscnt1, dir);
			}
		}
...
if((eSoundMode==e_SOUND_MODE_LOUD)||(eSoundMode==e_SOUND_MODE_ONCE))
	...
	case 919: //! M919 - Configure TMC2130 chopper
    {
		if (code_seen('O')) tmc2130_E_chopper_config.toff = code_value();
		if (code_seen('H')) tmc2130_E_chopper_config.hstrt = code_value();
		if (code_seen('E')) tmc2130_E_chopper_config.hend = code_value();
		if (code_seen('D')) tmc2130_E_chopper_config.disfdcc = code_value();
		if (code_seen('R')) tmc2130_E_chopper_config.rndtf = code_value();
		if (code_seen('C')) tmc2130_E_chopper_config.chm = code_value();
		if (code_seen('B')) tmc2130_E_chopper_config.tbl = code_value();
		if (code_seen('S')) tmc2130_E_chopper_config.sync = code_value();
		if (code_seen('I')) tmc2130_E_chopper_config.intpol = code_value();

		tmc2130_wr_CHOPCONF(E_AXIS,
							tmc2130_E_chopper_config.toff,
							tmc2130_E_chopper_config.hstrt,
							tmc2130_E_chopper_config.hend,
							tmc2130_E_chopper_config.fd3,
							tmc2130_E_chopper_config.disfdcc,
							tmc2130_E_chopper_config.rndtf,
							tmc2130_E_chopper_config.chm,
							tmc2130_E_chopper_config.tbl,
							1, // vsense
							0, // vighfs
							0, // vhighchm
							tmc2130_E_chopper_config.sync,
							tmc2130_mres[E_AXIS],
							tmc2130_E_chopper_config.intpol,
							0, // dedge
							0); // diss2g
    }
    break;
